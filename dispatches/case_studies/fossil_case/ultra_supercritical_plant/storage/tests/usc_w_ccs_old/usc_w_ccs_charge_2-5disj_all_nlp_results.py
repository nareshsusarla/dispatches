...
def view_result(outfile, m):
    ...
    # Steam to CCS
    tags['ccs_Fin'] = ("%4.3f" % (value(
        m.fs.ccs_reformer.inlet.flow_mol[0])*1e-3))
    tags['ccs_Tin'] = ("%4.2f" % (value(
        m.fs.ccs_reformer.control_volume.properties_in[0].temperature)))
    tags['ccs_Pin'] = ("%4.1f" % (value(
        m.fs.ccs_reformer.inlet.pressure[0])*1e-6))
    tags['ccs_Hin'] = ("%4.1f" % (value(
        m.fs.ccs_reformer.inlet.enth_mol[0])*1e-3))
    tags['ccs_xin'] = ("%4.4f" % (value(
        m.fs.ccs_reformer.control_volume.properties_in[0].vapor_frac)))

    # Steam out of CCS
    tags['ccs_Fout'] = ("%4.3f" % (value(
        m.fs.ccs_reformer.outlet.flow_mol[0])*1e-3))
    tags['ccs_Tout'] = ("%4.2f" % (value(
        m.fs.ccs_reformer.control_volume.properties_out[0].temperature)))
    tags['ccs_Pout'] = ("%4.1f" % (value(
        m.fs.ccs_reformer.outlet.pressure[0])*1e-6))
    tags['ccs_Hout'] = ("%4.1f" % (value(
        m.fs.ccs_reformer.outlet.enth_mol[0])*1e-3))
    tags['ccs_xout'] = ("%4.4f" % (value(
        m.fs.ccs_reformer.control_volume.properties_out[0].vapor_frac)))

    # Charge heat exchanger
    # if m.fs.charge.solar_salt_disjunct.binary_indicator_var == 1:
    ...
    tags['hxsteam_Pout'] = ("%4.4f" % (value(
        m.fs.charge.solar_salt_disjunct.hxc.outlet_1.pressure[0])*1e-6))
    tags['hxsteam_Hout'] = ("%4.2f" % (value(
        m.fs.charge.solar_salt_disjunct.hxc.outlet_1.enth_mol[0])))
    tags['hxsteam_xout'] = ("%4.4f" % (value(
        m.fs.charge.solar_salt_disjunct.hxc.side_1.
        properties_out[0].vapor_frac)))
    # elif m.fs.charge.hitec_salt_disjunct.binary_indicator_var == 1:
    #     tags['hxsteam_Fout'] = ("%4.4f" % (value(
    ...
    #         properties_out[0].temperature)))
    #     tags['hxsteam_Pout'] = ("%4.4f" % (value(
    #         m.fs.charge.hitec_salt_disjunct.hxc.outlet_1.pressure[0])*1e-6))
    #     tags['hxsteam_Hout'] = ("%4.2f" % (value(
    #         m.fs.charge.hitec_salt_disjunct.hxc.outlet_1.enth_mol[0])))
    #     tags['hxsteam_xout'] = ("%4.4f" % (value(
    #         m.fs.charge.hitec_salt_disjunct.hxc.side_1.
    #         properties_out[0].vapor_frac)))
    # else:
    #     tags['hxsteam_Fout'] = ("%4.4f" % (value(
    #         m.fs.charge.thermal_oil_disjunct.hxc.outlet_1.flow_mol[0])*1e-3))
    #     tags['hxsteam_Tout'] = ("%4.4f" % (value(
    #         m.fs.charge.thermal_oil_disjunct.hxc.side_1.
    #         properties_out[0].temperature)))
    #     tags['hxsteam_Pout'] = ("%4.4f" % (value(
    #         m.fs.charge.thermal_oil_disjunct.hxc.outlet_1.pressure[0])*1e-6))
    #     tags['hxsteam_Hout'] = ("%4.2f" % (value(
    #         m.fs.charge.thermal_oil_disjunct.hxc.outlet_1.enth_mol[0])))
    #     tags['hxsteam_xout'] = ("%4.4f" % (value(
    #         m.fs.charge.thermal_oil_disjunct.hxc.side_1.
    #         properties_out[0].vapor_frac)))
...
if __name__ == "__main__":
    ...
    for i in source_data:
        for j in fluid_data:
            for k in heat_duty_data:
                ...
                m, results = model_analysis(m_chg, solver, heat_duty=k, fluid=j, source=i)
                # View results in a process flow diagram
                view_result(i+'_'+j+'_'+str(k)+"_pfd_usc_gdp_2-5_results.svg", m)

                boiler_flow.append(value(m.fs.boiler.inlet.flow_mol[0]))
                net_power.append(value(m.fs.plant_power_out[0]))
                heat_load.append(value((m.fs.boiler.heat_duty[0]
                                        + m.fs.reheater[1].heat_duty[0]
                                        + m.fs.reheater[2].heat_duty[0])
                                       * 1e-6))
                flow_to_ccs.append(value(m.fs.ccs_reformer.inlet.flow_mol[0]))
                ccs_duty.append(value(m.fs.ccs_reformer.heat_duty[0]))

                objective.append(value(m.obj) * 1e-6)
                capex.append(value(m.fs.charge.plant_capital_cost) * 1e-6)
                opex.append(value(m.fs.charge.plant_fixed_operating_cost) * 1e-6)

                salt_choice.append(j)
                steam_source.append(i)
                storage_capacity.append(k)
                solverstatus.append(results[1].solver.termination_condition)
                if m.fs.charge.solar_salt_disjunct.binary_indicator_var.value == 1:
                    salt_flow_in.append(value(m.fs.charge.solar_salt_disjunct.hxc.inlet_2.flow_mass[0]))
                    salt_temp_in.append(value(m.fs.charge.solar_salt_disjunct.hxc.inlet_2.temperature[0]))
                    salt_temp_out.append(value(m.fs.charge.solar_salt_disjunct.hxc.outlet_2.temperature[0]))
                    steam_flow_in.append(value(m.fs.charge.solar_salt_disjunct.hxc.inlet_1.flow_mol[0]))
                    steam_temp_in.append(value(m.fs.charge.solar_salt_disjunct.hxc.side_1.properties_in[0].temperature))
                    steam_temp_out.append(value(m.fs.charge.solar_salt_disjunct.hxc.side_1.properties_out[0].temperature))
                    hxc_area.append(value(m.fs.charge.solar_salt_disjunct.hxc.area))
                    htc.append(value(m.fs.charge.solar_salt_disjunct.hxc.overall_heat_transfer_coefficient[0]))
                    delta_tin.append(value(m.fs.charge.solar_salt_disjunct.hxc.delta_temperature_in[0]))
                    delta_tout.append(value(m.fs.charge.solar_salt_disjunct.hxc.delta_temperature_out[0]))
                elif m.fs.charge.hitec_salt_disjunct.binary_indicator_var.value == 1:
                    salt_flow_in.append(value(m.fs.charge.hitec_salt_disjunct.hxc.inlet_2.flow_mass[0]))
                    salt_temp_in.append(value(m.fs.charge.hitec_salt_disjunct.hxc.inlet_2.temperature[0]))
                    salt_temp_out.append(value(m.fs.charge.hitec_salt_disjunct.hxc.outlet_2.temperature[0]))
                    steam_flow_in.append(value(m.fs.charge.hitec_salt_disjunct.hxc.inlet_1.flow_mol[0]))
                    steam_temp_in.append(value(m.fs.charge.hitec_salt_disjunct.hxc.side_1.properties_in[0].temperature))
                    steam_temp_out.append(value(m.fs.charge.hitec_salt_disjunct.hxc.side_1.properties_out[0].temperature))
                    hxc_area.append(value(m.fs.charge.hitec_salt_disjunct.hxc.area))
                    htc.append(value(m.fs.charge.hitec_salt_disjunct.hxc.overall_heat_transfer_coefficient[0]))
                    delta_tin.append(value(m.fs.charge.hitec_salt_disjunct.hxc.delta_temperature_in[0]))
                    delta_tout.append(value(m.fs.charge.hitec_salt_disjunct.hxc.delta_temperature_out[0]))
                else:
                    salt_flow_in.append(value(m.fs.charge.thermal_oil_disjunct.hxc.inlet_2.flow_mass[0]))
                    salt_temp_in.append(value(m.fs.charge.thermal_oil_disjunct.hxc.inlet_2.temperature[0]))
                    salt_temp_out.append(value(m.fs.charge.thermal_oil_disjunct.hxc.outlet_2.temperature[0]))
                    steam_flow_in.append(value(m.fs.charge.thermal_oil_disjunct.hxc.inlet_1.flow_mol[0]))
                    steam_temp_in.append(value(m.fs.charge.thermal_oil_disjunct.hxc.side_1.properties_in[0].temperature))
                    steam_temp_out.append(value(m.fs.charge.thermal_oil_disjunct.hxc.side_1.properties_out[0].temperature))
                    hxc_area.append(value(m.fs.charge.thermal_oil_disjunct.hxc.area))
                    htc.append(value(m.fs.charge.thermal_oil_disjunct.hxc.overall_heat_transfer_coefficient[0]))
                    delta_tin.append(value(m.fs.charge.thermal_oil_disjunct.hxc.delta_temperature_in[0]))
                    delta_tout.append(value(m.fs.charge.thermal_oil_disjunct.hxc.delta_temperature_out[0]))
    df = pd.DataFrame(
        {
            "Choice of Salt": salt_choice,
            "Steam Source": steam_source,
            "Storage Capacity": storage_capacity,
            "Boiler Flow [mol/s]": boiler_flow,
            "Net Power [MW]": net_power,
            "Plant Heat Load [MWth]": heat_load,
            "Steam to CCS [mol/s]": flow_to_ccs,
            "CCS Duty [W]": ccs_duty,
            "Objective": objective,
            "Annual Capital Cost": capex,
            "Annual Operating Cost": opex,
            "Salt Flow In [kg/s]": salt_flow_in,
            "Salt Temperature In [K]": salt_temp_in,
            "Salt Temperature Out [K]": salt_temp_out,
            "Steam Flow In [mol/s]": steam_flow_in,
            "Steam Temperature In [K]": steam_temp_in,
            "Steam Temperature Out [K]": steam_temp_out,
            "Delta Temperature In": delta_tin,
            "Delta Temperature Out": delta_tout,
            "HXC Area": hxc_area,
    ...
    df.to_csv('USC_sensitivity_update.csv', index=False)
