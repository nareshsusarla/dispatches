...
def thermal_oil_disjunct_equations(disj):
    ...
    # m.fs.charge.thermal_oil_disjunct.connector_to_hxc_flow_mol = Constraint(
    #     expr=m.fs.charge.thermal_oil_disjunct.hxc.inlet_1.flow_mol[0] ==
    #     m.fs.charge.connector.outlet.flow_mol[0],
    #     doc="Connection from connector to thermal oil charge heat exchanger"
    # )
    # m.fs.charge.thermal_oil_disjunct.connector_to_hxc_enth_mol = Constraint(
    #     expr=m.fs.charge.thermal_oil_disjunct.hxc.inlet_1.enth_mol[0] ==
    #     m.fs.charge.connector.outlet.enth_mol[0],
    #     doc="Connection from connector to thermal oil charge heat exchanger"
    # )
    # m.fs.charge.thermal_oil_disjunct.connector_to_hxc_press = Constraint(
    #     expr=m.fs.charge.thermal_oil_disjunct.hxc.inlet_1.pressure[0] ==
    #     m.fs.charge.connector.outlet.pressure[0],
    #     doc="Connection from connector to thermal oil charge heat exchanger"
    # )
...
def add_bounds(m):
    ...
    m.fs.charge.hitec_salt_disjunct.hxc.delta_temperature_out.setlb(9.7)
    ...
    for oil_hxc in [m.fs.charge.thermal_oil_disjunct.hxc]:
        ...
        oil_hxc.delta_temperature_in.setlb(9.5)  # K
        # oil_hxc.delta_temperature_in.setub(552.5)  # works for 100MW hxc
        oil_hxc.delta_temperature_in.setub(556)  # works for 150MW hxc
    ...
    m.fs.charge.capital_cost.setub(1e11)
...
def print_model(nlp_model, nlp_data):
    ...
    if nlp_model.fs.charge.solar_salt_disjunct.binary_indicator_var.value == 1:
        ...
        print('        Heat exchanger OHTC: {:.4f}'.format(
            value(nlp_model.fs.charge.solar_salt_disjunct.hxc.
                  overall_heat_transfer_coefficient[0])))
        print('        Tank volume (m3): {:.4f}'.format(
            value(nlp_model.fs.charge.solar_salt_disjunct.tank_volume)))
        ...
        print('        Heat exchanger OHTC: {:.4f}'.format(
            value(nlp_model.fs.charge.hitec_salt_disjunct.hxc.
                  overall_heat_transfer_coefficient[0])))
        print('        Tank volume (m3): {:.4f}'.format(
            value(nlp_model.fs.charge.hitec_salt_disjunct.tank_volume)))
    else:
        ...
        print('        Heat exchanger OHTC: {:.4f}'.format(
            value(nlp_model.fs.charge.thermal_oil_disjunct.hxc.
                  overall_heat_transfer_coefficient[0])))
        print('        Tank volume (m3): {:.4f}'.format(
            value(nlp_model.fs.charge.thermal_oil_disjunct.tank_volume)))
    ...
    # nlp_model.fs.charge.thermal_oil_disjunct.display()
