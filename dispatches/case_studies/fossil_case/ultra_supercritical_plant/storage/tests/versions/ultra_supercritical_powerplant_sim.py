...
def declare_unit_model():
    ...
    # m.fs.ess_ip_split = Separator(
    #     default={
    #         "property_package": m.fs.prop_water,
    #         "material_balance_type": MaterialBalanceType.total,
    #         "split_basis": SplittingType.totalFlow,
    #         "ideal_separation": False,
    #         "outlet_list": ["to_hxc", "to_ip"],
    #         "has_phase_equilibrium": False
    #     }
    # )

    # ###########################################################################
    # #  Add dummy heat exchanger                                               #
    # ###########################################################################
    # # A connector model is defined as a dummy heat exchanger
    # # with Q=0 and a deltaP=0
    # m.fs.connector = Heater(
    #     default={
    #         "dynamic": False,
    #         "property_package": m.fs.prop_water,
    #         "has_pressure_change": False
...
def _make_constraints(m):
    ...
    # #-------- added by esrawli
    # # Connecting the ess ip splitter to the connector using constraints
    # # instead of arcs. Note that these constraints should be deactivated
    # # when the disjunctions are included
    # @m.fs.connector.Constraint(m.fs.time)
    # def constraint_connector_flow(b, t):
    #     return b.inlet.flow_mol[t] == m.fs.ess_ip_split.to_hxc.flow_mol[t]

    # @m.fs.connector.Constraint(m.fs.time)
    # def constraint_connector_pres(b, t):
    #     return b.inlet.pressure[t] == m.fs.ess_ip_split.to_hxc.pressure[t]

    # @m.fs.connector.Constraint(m.fs.time)
    # def constraint_connector_enth(b, t):
    #     return b.inlet.enth_mol[t] == m.fs.ess_ip_split.to_hxc.enth_mol[t]
    # #--------
...
def _create_arcs(m):
    ...
    # Connecting the reheater to the ess ip splitter instead
    # of the turbine 3
    # m.fs.reheater1_to_essip = Arc(
    #     source=m.fs.reheater_1.outlet,
    #     destination=m.fs.ess_ip_split.inlet
    # )
    # m.fs.essip_to_turb3 = Arc(
    #     source=m.fs.ess_ip_split.to_ip,
    #     destination=m.fs.turbine[3].inlet
    # )
    #--------
    # turbine3 splitter
    ...
    # #-------- added by esrawli
    # # Connecting the ess ip splitter to the connector.
    # # Note that this arc should be erased 
    # # when the disjunctions are included
    # m.fs.essip_to_connector = Arc(
    #     source=m.fs.ess_ip_split.to_hxc,
    #     destination=m.fs.connector.inlet
    # )
    # #--------
...
def set_model_input(m):
    ...
    # m.fs.ess_ip_split.split_fraction[0, "to_hxc"].fix(0.01)

    # ###########################################################################
    # #  Connector                                                              #
    # ###########################################################################
    # m.fs.connector.heat_duty[0].fix(0) # Fix heat duty to zero for dummy conector
    #--------
...
def initialize(m, fileinput=None, outlvl=idaeslog.NOTSET,
    ...
    # #-------- added by esrawli
    # #  Intermediate pressure splitter initialization
    # _set_port(m.fs.ess_ip_split.inlet,  m.fs.reheater_1.outlet)
    # m.fs.ess_ip_split.initialize(outlvl=outlvl, optarg=solver.options)
    # #--------

    #-------- modified by esrawli
    # Connecting the turbine 3 inlet to the ess ip splitter
    # instead of the reheater 1
    _set_port(m.fs.turbine[3].inlet, m.fs.reheater_1.outlet)
    # _set_port(m.fs.turbine[3].inlet, m.fs.ess_ip_split.to_ip)
    ...
    # _set_port(m.fs.connector.inlet, m.fs.ess_ip_split.to_hxc)
    # # m.fs.connector.inlet.flow_mol.fix()
    # # m.fs.connector.inlet.enth_mol.fix()
    # # m.fs.connector.inlet.pressure.fix()
    # m.fs.connector.initialize(outlvl=outlvl, optarg=solver.options)
    # #--------
...
def add_bounds(m):
    ...
    # The NLP iterates super slow when added to the model (simulation model)
...
def build_plant_model(initialize_from_file=None, store_initialization=None):
    ...
    # m.fs.plant_power_out[:].fix(437)
    
    # Unfix the steam split fraction to charge heat exchanger
    # m.fs.ess_hp_split.split_fraction[0, "to_hxc"].unfix()  
    # m.fs.ess_ip_split.split_fraction[0, "to_hxc"].unfix()
    m.fs.ess_hp_split.split_fraction[0, "to_hxc"].fix(0)  
    # m.fs.ess_ip_split.split_fraction[0, "to_hxc"].fix(0)

    # Unfix the connector inlets
    # for unit in [m.fs.connector]:
...
#   At this time one can make chnages to the model for further analysis
    ...
    for i in flow_frac_list:
        for j in pres_frac_list:
            ...
            #       pyo.value(m.obj))
            # print("Total capital cost (MM$/y):",
            #       pyo.value(m.fs.capital_cost * 1e-6))
            # print("Operating cost (MM$/y):",
            #       pyo.value(m.fs.operating_cost * 1e-6))
            print("")
