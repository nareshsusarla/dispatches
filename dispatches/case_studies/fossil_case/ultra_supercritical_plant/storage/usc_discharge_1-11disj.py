...
def _make_constraints(m, add_efficiency=None, power_max=None):
    ...
    #     return b.discharge.hxd.shell_inlet.flow_mass[t] <= 500
...
def set_model_input(m):
    ...
    m.fs.discharge.hxd.area.fix(2000)  # m2 **500
    ...
    # optimization
    m.fs.discharge.hxd.shell_inlet.flow_mass.fix(200)   # kg/s
    m.fs.discharge.hxd.shell_inlet.temperature.fix(831.15)  # K **831.15 **828.59
    m.fs.discharge.hxd.shell_inlet.pressure.fix(101325)  # Pa
    ...
    m.fs.discharge.es_split.inlet.flow_mol.fix(17854)   # kg/s **19840
    ...
    m.fs.discharge.es_split.split_fraction[0, "to_hxd"].fix(0.2) # **0.1
...
def initialize(m, solver=None,
    ...
    m.fs.discharge.hxd.initialize(outlvl=outlvl,
                                  optarg=optarg)
    m.fs.discharge.hxd.report()
    propagate_state(m.fs.discharge.hxd_to_esturbine)
    print(" ")
    print("Degree of Freedom for Turbine ", degrees_of_freedom(m.fs.discharge.es_turbine))
    # m.fs.discharge.es_turbine.inlet.unfix()
    m.fs.discharge.es_turbine.pprint()
    m.fs.discharge.es_turbine.initialize(outlvl=idaeslog.INFO_HIGH,
                                         optarg=optarg)
...
def build_costing(m,
    ...
    m.fs.discharge.spump_Qgpm = pyo.Expression(
        expr=(m.fs.discharge.hxd.hot_side.properties_in[0].flow_mass *
...
def print_results(m, results):
    ...
    print('Obj ($/hr): {:.6f}'.format(value(m.obj) / scaling_obj))
    ...
    print(' Cost ($/yr): {:.6f}'.format(pyo.value(hxd.costing.capital_cost /
                                                  m.fs.discharge.num_of_years)))
    print(' Steam flow to storage (mol/s): {:.6f}'.format(pyo.value(hxd.tube_inlet.flow_mol[0])))
    print(' Salt flow (kg/s): {:.6f}'.format(pyo.value(hxd.shell_inlet.flow_mass[0])))
    print(' Water temperature in/out (K): {:.6f}/{:.6f}'.format(
        pyo.value(hxd.cold_side.properties_in[0].temperature),
        pyo.value(hxd.cold_side.properties_out[0].temperature)))
    print(' Salt temperature in/out (K): {:.6f}/{:.6f}'.format(
        pyo.value(hxd.shell_inlet.temperature[0]),
        pyo.value(hxd.shell_outlet.temperature[0])))
    ...
    print(' Salt dens_mass: {:.6f}'.format(
        pyo.value(hxd.hot_side.properties_in[0].dens_mass['Liq'])))
    print('')
...
def model_analysis(m, heat_duty=None, source=None):
...
if __name__ == "__main__":
    ...
    #           "fwh3", "fwh2",
    #           "fwh1"]
    m = model_analysis(m, heat_duty=148.5, source=None)
