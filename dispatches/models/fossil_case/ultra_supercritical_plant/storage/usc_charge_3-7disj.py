...
def create_charge_model(m):
    ...
    # Commented since recycle mixer is included in sink disjunction 3,
    # disjunct 1
    ###########################################################################
    #  Add recycle mixer                                                      #
    ###########################################################################
    ...
    # Disjunction 3 for the sink selection consists of 2 disjuncts:
    #   1. recycle_mix_sink_disjunct ===> return condensed steam to recycle mixer
    #   2. mix8_sink_disjunct ===> return condensed steam to mixer 8 
    ...
    m.fs.charge.recycle_mix_sink_disjunct = Disjunct(
        rule=recycle_mix_sink_disjunct_equations)
    m.fs.charge.mix8_sink_disjunct = Disjunct(
        rule=mix8_sink_disjunct_equations)
...
def _create_arcs(m):
    ...
    # Arcs deactivated for sink disjunction 3. **NOTE** When included, 3
    # DOFs appear in def initialize
    for arc_s in [m.fs.t2split_to_fwh8mix]:
        arc_s.expanded_block.enth_mol_equality.deactivate()
        arc_s.expanded_block.flow_mol_equality.deactivate()
        arc_s.expanded_block.pressure_equality.deactivate()
    ...
    #-------- commented by esrawli
    # Commented since the recycle mixer is now included in sink
    # disjunction 3, disjunct 1
...
def add_disjunction(m):
    ...
    # Add disjunction 3 for the sink selection
    m.fs.sink_disjunction = Disjunction(
        expr=[m.fs.charge.recycle_mix_sink_disjunct,
              m.fs.charge.mix8_sink_disjunct]
    )
...
def recycle_mix_sink_disjunct_equations(disj):
    m = disj.model()

    m.fs.charge.recycle_mix_sink_disjunct.recycle_mixer = Mixer(
    ...
    # Recycle mixer minimum pressure constraint
    m.fs.charge.recycle_mix_sink_disjunct.recyclemixer_pressure_constraint = Constraint(
        expr=m.fs.charge.recycle_mix_sink_disjunct.recycle_mixer.from_bfw_out_state[0].pressure == \
        m.fs.charge.recycle_mix_sink_disjunct.recycle_mixer.mixed_state[0].pressure
    )

    # Define arcs to connect recycle mixer to storage system
    m.fs.charge.recycle_mix_sink_disjunct.hxpump_to_recyclemix = Arc(
        source=m.fs.charge.hx_pump.outlet,
        destination=m.fs.charge.recycle_mix_sink_disjunct.recycle_mixer.from_hx_pump,
        doc="Connection from HX pump to recycle mixer"
    )
    m.fs.charge.recycle_mix_sink_disjunct.bfp_to_recyclemix = Arc(
        source=m.fs.bfp.outlet,
        destination=m.fs.charge.recycle_mix_sink_disjunct.recycle_mixer.from_bfw_out,
        doc="Connection from BFP outlet to recycle mixer"
    )
    m.fs.charge.recycle_mix_sink_disjunct.recyclemix_to_fwh8 = Arc(
        source=m.fs.charge.recycle_mix_sink_disjunct.recycle_mixer.outlet,
        destination=m.fs.fwh[8].inlet_2,
        doc="Connection from Recycle Mixer to FWH8 tube side"
    )

    # Re-connect turbine 2 spliter to FWH8 mixer
    m.fs.charge.recycle_mix_sink_disjunct.t2split_to_fwh8mix = Arc(
        source=m.fs.turbine_splitter[2].outlet_2,
        destination=m.fs.fwh_mixer[8].steam
    )


def mix8_sink_disjunct_equations(disj):
    m = disj.model()

    m.fs.charge.mix8_sink_disjunct.mixer8 = Mixer(
        default={"momentum_mixing_type": MomentumMixingType.none,
                 "material_balance_type": MaterialBalanceType.componentTotal,
                 "inlet_list": ["from_turbine", "from_hx_pump"],
                 "property_package": m.fs.prop_water}
    )

    # Recycle mixer minimum pressure constraint
    m.fs.charge.mix8_sink_disjunct.mix8_pressure_constraint = Constraint(
        expr=m.fs.charge.mix8_sink_disjunct.mixer8.from_turbine_state[0].pressure == \
        m.fs.charge.mix8_sink_disjunct.mixer8.mixed_state[0].pressure
    )

    # Define arcs to connect mixer 8 to the flowsheet
    m.fs.charge.mix8_sink_disjunct.hxpump_to_mix8 = Arc(
        source=m.fs.charge.hx_pump.outlet,
        destination=m.fs.charge.mix8_sink_disjunct.mixer8.from_hx_pump,
        doc="Connection from HX pump to mixer 8"
    )
    m.fs.charge.mix8_sink_disjunct.t2split_to_mix8 = Arc(
        source=m.fs.turbine_splitter[2].outlet_2,
        destination=m.fs.charge.mix8_sink_disjunct.mixer8.from_turbine,
        doc="Connection from turbine 2 splitter to mixer 8"
    )
    m.fs.charge.mix8_sink_disjunct.mix8_to_fwh8mix = Arc(
        source=m.fs.charge.mix8_sink_disjunct.mixer8.outlet,
        destination=m.fs.fwh_mixer[8].steam,
        doc="Connection from mixer 8 to FWH8 mix"
    )

    # Re-connect BFW to FWH8
    m.fs.charge.mix8_sink_disjunct.bfp_to_fwh8 = Arc(
        source=m.fs.bfp.outlet,
        destination=m.fs.fwh[8].inlet_2
    )
...
def initialize(m, solver=None, outlvl=idaeslog.NOTSET,
    ...
    propagate_state(m.fs.charge.boiler_to_essvhp)
    ...
    #-------- modified by esrawli
    # The recycle mixer is a unit declared inside a disjunct and
    # initialized here
    # Recycle mixer initialization
    propagate_state(m.fs.charge.recycle_mix_sink_disjunct.bfp_to_recyclemix)
    propagate_state(m.fs.charge.recycle_mix_sink_disjunct.hxpump_to_recyclemix)
    m.fs.charge.recycle_mix_sink_disjunct.recycle_mixer.initialize(outlvl=outlvl)
    #--------

    #-------- added by esrawli
    # # Re-initialize FWH8 since it is now connected to the recycle mixer
    print('DOFs fwh8 before init =', degrees_of_freedom(m.fs.fwh[8]))
    # m.fs.fwh[8].display()
    # m.fs.fwh[8].inlet_1.report()
    propagate_state(m.fs.charge.recycle_mix_sink_disjunct.recyclemix_to_fwh8)
    # m.fs.fwh[8].inlet_2.fix() # DOFs zero when uncommented
    m.fs.fwh[8].initialize(outlvl=outlvl, optarg=solver.options)

    # Re-initialize FWH8 mixer since I disconnected the turbine 2
    propagate_state(m.fs.charge.recycle_mix_sink_disjunct.t2split_to_fwh8mix)
    m.fs.fwh_mixer[8].steam.fix()
    m.fs.fwh_mixer[8].initialize(outlvl=outlvl, optarg=solver.options)

    # Initialize mixer 8 in sink disjunction, disjunct 2
    propagate_state(m.fs.charge.mix8_sink_disjunct.t2split_to_mix8)
    propagate_state(m.fs.charge.mix8_sink_disjunct.hxpump_to_mix8)
    m.fs.charge.mix8_sink_disjunct.mixer8.initialize(outlvl=outlvl)#, optarg=solver.options)
    #--------
...
def add_bounds(m):
    ...
    # ----- Tin & Tout upper bound value works for esrawli (End) -----
    ...
    for oil_hxc in [m.fs.charge.thermal_oil_disjunct.hxc]:
        ...
        oil_hxc.delta_temperature_out.setlb(9.70)  # works for esrawli
    ...
    #--------- added by esrawli
    # Bounds needed for disjunction 3
    m.fs.turbine_splitter[2].outlet_2.flow_mol[:].setlb(0)
    m.fs.turbine_splitter[2].outlet_2.flow_mol[:].setub(m.flow_max * 0.20)

    for mix in [m.fs.charge.recycle_mix_sink_disjunct.recycle_mixer]:
    ...
    for mix in [m.fs.charge.mix8_sink_disjunct.mixer8]:
        mix.from_turbine.flow_mol.setlb(0)
        mix.from_turbine.flow_mol.setub(m.flow_max)
        mix.from_hx_pump.flow_mol.setlb(0)
        mix.from_hx_pump.flow_mol.setub(0.2* m.flow_max)
        mix.outlet.flow_mol.setlb(0)
        mix.outlet.flow_mol.setub(m.flow_max)
...
def print_model(nlp_model, nlp_data):
    ...
    for k in nlp_model.set_turbine_splitter:
        print('Turbine {} split fraction out 1: {}'.format(
            k, value(nlp_model.fs.turbine_splitter[k].split_fraction[0.0, "outlet_1"])))
        print('Turbine {} split fraction out 2: {}'.format(
            k, value(nlp_model.fs.turbine_splitter[k].split_fraction[0.0, "outlet_2"])))
...
def model_analysis(m, solver):
    ...
    #-------- addded by esrawli
    # Unfix data for disjunction 3 fixed during initialization
    m.fs.fwh_mixer[8].steam.unfix()
    ...
    #------- added by esrawli
    # Fix disjuncts in disjunction 3 to solve the recycle_mixer case
    m.fs.charge.recycle_mix_sink_disjunct.indicator_var.fix(1)
    m.fs.charge.mix8_sink_disjunct.indicator_var.fix(0)
    #--------
