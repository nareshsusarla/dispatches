...
def thermal_oil_disjunct_equations(disj):
    ...
    def constraint_hxc_ohtc_thermal_oil(b, t):
        ...
        #     == 1 / ((1 / m.fs.charge.thermal_oil_disjunct.hxc.h_salt)
        #             + ((m.fs.charge.thermal_oil_disjunct.hxc.tube_outer_dia *
...
def set_scaling_factors(m):
    ...
    for fluid in [m.fs.charge.solar_salt_disjunct.hxc,
        ...
        iscale.set_scaling_factor(fluid.shell.properties_in[0].flow_mol, 1e-3) # added by esrawli
        iscale.set_scaling_factor(fluid.tube.properties_in[0].flow_mass, 1e-2)
        iscale.set_scaling_factor(fluid.shell.properties_out[0].flow_mol, 1e-3) # added by esrawli
        iscale.set_scaling_factor(fluid.tube.properties_out[0].flow_mass, 1e-2) # added by esrawli
        iscale.set_scaling_factor(fluid.shell.properties_in[0].pressure, 1e-6) # added by esrawli
        iscale.set_scaling_factor(fluid.tube.properties_in[0].pressure, 1e-6) # added by esrawli
        iscale.set_scaling_factor(fluid.shell.properties_out[0].pressure, 1e-6) # added by esrawli
        iscale.set_scaling_factor(fluid.tube.properties_out[0].pressure, 1e-6) # added by esrawli

    iscale.set_scaling_factor(m.fs.charge.hx_pump.control_volume.work, 1e-6)
    iscale.set_scaling_factor(
        m.fs.charge.hx_pump.control_volume.properties_in[0].pressure, 1e-6) # added by esrawli
    iscale.set_scaling_factor(
        m.fs.charge.hx_pump.control_volume.properties_out[0].pressure, 1e-6) # added by esrawli

    for k in [m.fs.charge.cooler, m.fs.charge.connector]:
        iscale.set_scaling_factor(k.control_volume.heat, 1e-6)
    iscale.set_scaling_factor(k.control_volume.properties_in[0].pressure, 1e-6) # added by esrawli
    iscale.set_scaling_factor(k.control_volume.properties_out[0].pressure, 1e-6) # added by esrawli

    #-------- added by esrawli
    for k in m.set_turbine:
        a = m.fs.turbine[k]
        iscale.set_scaling_factor(a.control_volume.properties_in[0].pressure, 1e-6)
        iscale.set_scaling_factor(a.control_volume.properties_out[0].pressure, 1e-6)
        iscale.set_scaling_factor(a.control_volume.properties_in[0].enth_mol, 1e-3)
        iscale.set_scaling_factor(a.control_volume.properties_out[0].enth_mol, 1e-3)
    for k in m.set_fwh:
        b = m.fs.fwh[k]
    ...
    for j in [m.fs.boiler, m.fs.reheater[1], m.fs.reheater[2], m.fs.bfp, m.fs.bfpt]:
        iscale.set_scaling_factor(j.control_volume.properties_in[0].pressure, 1e-6)
        iscale.set_scaling_factor(j.control_volume.properties_out[0].pressure, 1e-6)
        iscale.set_scaling_factor(j.control_volume.properties_in[0].enth_mol, 1e-3)
        iscale.set_scaling_factor(j.control_volume.properties_out[0].enth_mol, 1e-3)
    #--------
...
def add_bounds(m):
    ...
    for oil_hxc in [m.fs.charge.thermal_oil_disjunct.hxc]:
        ...
        oil_hxc.heat_duty.setub(300e6) # increasing the value from 200 to 300
        oil_hxc.shell.heat.setlb(-300e6) # increasing the value from 200 to 300
        oil_hxc.shell.heat.setub(0)
        oil_hxc.tube.heat.setlb(0)
        oil_hxc.tube.heat.setub(300e6) # increasing the value from 200 to 300
