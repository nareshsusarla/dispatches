...
def create_charge_model(m):
    ...
    m.fs.charge.ess_ip_split = Separator(
        default={
            "property_package": m.fs.prop_water,
            "material_balance_type": MaterialBalanceType.total,
            "split_basis": SplittingType.totalFlow,
    ...
    #   3. thermal_oil_disjunct ======> thermal oil used as the storage medium
    # Disjunction 2 for the steam source selection consists of 2 disjuncts:
    #   1. vhp_source_disjunct =======> very high pressure steam used as heat source
    #   2. hp_source_disjunct =======> high pressure steam used as heat source
    #   3. ip_source_disjunct =======> intermediate pressure steam used as heat source
    ...
    m.fs.charge.thermal_oil_disjunct = Disjunct(rule=thermal_oil_disjunct_equations)
    ...
    m.fs.charge.ip_source_disjunct = Disjunct(rule=ip_source_disjunct_equations)
...
def _create_arcs(m):
    ...
    # Connect boiler to VHP splitter
    m.fs.charge.boiler_to_essvhp = Arc(
    ...
    # Connect reheater 1 to HP splitter
    ...
    # Connect reheater 2 to IP splitter
    m.fs.charge.rh2_to_essip = Arc(
        source=m.fs.reheater[2].outlet,
        destination=m.fs.charge.ess_ip_split.inlet,
        doc="Connection from reheater 2 to IP splitter"
    )
    m.fs.charge.essip_to_turb5 = Arc(
        source=m.fs.charge.ess_ip_split.to_turbine,
        destination=m.fs.turbine[5].inlet,
        doc="Connection from HP splitter to turbine 5"
    )
...
def thermal_oil_disjunct_equations(disj):
    ...
    m.fs.charge.thermal_oil_disjunct.hxc.oil_in_dynamic_viscosity = Expression(
        expr=m.fs.charge.thermal_oil_disjunct.hxc.side_2.properties_in[0].visc_kin *
    ...
    m.fs.charge.thermal_oil_disjunct.hxc.oil_out_dynamic_viscosity = Expression(
        expr=m.fs.charge.thermal_oil_disjunct.hxc.side_2.properties_out[0].visc_kin *
    ...
    m.fs.charge.thermal_oil_disjunct.hxc.steam_prandtl_number = Expression(
        expr=(
            (m.fs.charge.thermal_oil_disjunct.hxc.side_1.properties_in[0].cp_mol
             / m.fs.charge.thermal_oil_disjunct.hxc.side_1.properties_in[0].mw) * \
            m.fs.charge.thermal_oil_disjunct.hxc.side_1.properties_in[0].visc_d_phase["Vap"]
            / m.fs.charge.thermal_oil_disjunct.hxc.side_1.properties_in[0].
            therm_cond_phase["Vap"]
        ),
        doc="Steam Prandtl Number"
    ...
    m.fs.charge.thermal_oil_disjunct.hxc.steam_nusselt_number = Expression(
        expr=(
            0.023 * (m.fs.charge.thermal_oil_disjunct.hxc.steam_reynolds_number ** 0.8)
            * (m.fs.charge.thermal_oil_disjunct.hxc.steam_prandtl_number ** (0.33))
    ...
    m.fs.charge.thermal_oil_disjunct.hxc.h_oil = Expression(
        expr=(
            ...
            * m.fs.charge.thermal_oil_disjunct.hxc.
            oil_nusselt_number / m.fs.charge.thermal_oil_disjunct.hxc.tube_outer_dia
        ),
        doc="Salt side convective heat transfer coefficient [W/mK]"
    )
    m.fs.charge.thermal_oil_disjunct.hxc.h_steam = Expression(
        expr=(
            ...
            side_1.properties_in[0].therm_cond_phase["Vap"]
            * m.fs.charge.thermal_oil_disjunct.hxc.
            steam_nusselt_number / m.fs.charge.thermal_oil_disjunct.hxc.tube_inner_dia
        ),
        doc="Steam side convective heat transfer coefficient [W/mK]"
    )
    ...
    m.fs.charge.thermal_oil_disjunct.hxc.log_tube_dia_ratio = log(
        m.fs.charge.thermal_oil_disjunct.hxc.tube_dia_ratio)
    @m.fs.charge.thermal_oil_disjunct.hxc.Constraint(m.fs.time,
                                                     doc="Thermal oil charge heat exchanger\
                                                     overall heat transfer coefficient")
    def constraint_hxc_ohtc_thermal_oil(b, t):
        return (
            m.fs.charge.thermal_oil_disjunct.hxc.overall_heat_transfer_coefficient[t] * \
...
def vhp_source_disjunct_equations(disj):
    ...
    # Set lower and upper bounds of IP splitter to None and add
    # constraints instead
    m.fs.charge.ess_ip_split.split_fraction[0, "to_hxc"].setlb(None)
    m.fs.charge.ess_ip_split.split_fraction[0, "to_hxc"].setub(None)
    m.fs.charge.vhp_source_disjunct.split_vhp_lb_eq = Constraint(
        expr=m.fs.charge.ess_ip_split.split_fraction[0, "to_hxc"] >= 0,
        doc="Lower bound for the IP splitter"
    )
    m.fs.charge.vhp_source_disjunct.split_vhp_ub_eq = Constraint(
        expr=m.fs.charge.ess_ip_split.split_fraction[0, "to_hxc"] <= 1,
        doc="Upper bound for the IP splitter"
    )
    m.fs.charge.vhp_source_disjunct.split_vhp_eq = Constraint(
        expr=m.fs.charge.ess_ip_split.split_fraction[0, "to_hxc"] == 0,
        doc="No flow from IP split to charge heat exchanger"
    )
...
def hp_source_disjunct_equations(disj):
    ...
    m.fs.charge.ess_ip_split.split_fraction[0, "to_hxc"].setlb(None)
    m.fs.charge.ess_ip_split.split_fraction[0, "to_hxc"].setub(None)
    m.fs.charge.hp_source_disjunct.split_ip_lb_eq = Constraint(
        expr=m.fs.charge.ess_ip_split.split_fraction[0, "to_hxc"] >= 0,
        doc="Lower bound for the IP splitter"
    )
    m.fs.charge.hp_source_disjunct.split_ip_ub_eq = Constraint(
        expr=m.fs.charge.ess_ip_split.split_fraction[0, "to_hxc"] <= 1,
        doc="Upper bound for the IP splitter"
    )
    m.fs.charge.hp_source_disjunct.split_ip_eq = Constraint(
        expr=m.fs.charge.ess_ip_split.split_fraction[0, "to_hxc"] == 0,
        doc="No flow from IP split to charge heat exchanger"
    )
...
def ip_source_disjunct_equations(disj):
    """Disjunction 2: selection of intermediate pressure source
    """
    ...
    m.fs.charge.ip_source_disjunct.ipsplit_to_connector = Arc(
        source=m.fs.charge.ess_ip_split.to_hxc,
        destination=m.fs.charge.connector.inlet,
    ...
    m.fs.charge.ess_vhp_split.split_fraction[0, "to_hxc"].setlb(None)
    m.fs.charge.ess_vhp_split.split_fraction[0, "to_hxc"].setub(None)
    m.fs.charge.ip_source_disjunct.split_vhp_lb_eq = Constraint(
        expr=m.fs.charge.ess_vhp_split.split_fraction[0, "to_hxc"] >= 0,
        doc="Lower bound for the VHP splitter"
    )
    m.fs.charge.ip_source_disjunct.split_vhp_ub_eq = Constraint(
        expr=m.fs.charge.ess_vhp_split.split_fraction[0, "to_hxc"] <= 1,
        doc="Upper bound for the VHP splitter"
    )
    m.fs.charge.ip_source_disjunct.split_vhp_eq = Constraint(
        expr=m.fs.charge.ess_vhp_split.split_fraction[0, "to_hxc"] == 0,
        doc="No flow from VHP split to charge heat exchanger"
    )
    ...
    m.fs.charge.ess_hp_split.split_fraction[0, "to_hxc"].setlb(None)
    m.fs.charge.ess_hp_split.split_fraction[0, "to_hxc"].setub(None)
    m.fs.charge.ip_source_disjunct.split_hp_lb_eq = Constraint(
        expr=m.fs.charge.ess_hp_split.split_fraction[0, "to_hxc"] >= 0,
        doc="Lower bound for the HP splitter"
    )
    m.fs.charge.ip_source_disjunct.split_hp_ub_eq = Constraint(
        expr=m.fs.charge.ess_hp_split.split_fraction[0, "to_hxc"] <= 1,
        doc="Upper bound for the HP splitter"
    )
    m.fs.charge.ip_source_disjunct.split_hp_eq = Constraint(
        expr=m.fs.charge.ess_hp_split.split_fraction[0, "to_hxc"] == 0,
        doc="No flow from HP split to charge heat exchanger"
    )
...
def set_model_input(m):
    ...
    # Comment overall heat transfer coefficient since it is now
    # calculated
    # m.fs.charge.thermal_oil_disjunct.hxc.overall_heat_transfer_coefficient.fix(432.677) # from Andres's model
    m.fs.charge.thermal_oil_disjunct.hxc.inlet_2.flow_mass[0].fix(600) # from Andres's model
    m.fs.charge.thermal_oil_disjunct.hxc.inlet_2.temperature[0].fix(353.15) # from Andres's model
    m.fs.charge.thermal_oil_disjunct.hxc.inlet_2.pressure[0].fix(101325) # from Andres's model
    ...
    m.fs.charge.ess_ip_split.split_fraction[0, "to_hxc"].fix(0.01)
...
def initialize(m, solver=None, outlvl=idaeslog.NOTSET, optarg={"tol": 1e-8, "max_iter": 300}):
    ...
    print()
    print("****   Starting Charge Model Initialization ")
    print('DOFs at beginning of init =', degrees_of_freedom(m))
    ...
    _set_port(m.fs.charge.ess_ip_split.inlet,
              m.fs.reheater[2].outlet)
    m.fs.charge.ess_ip_split.initialize(outlvl=outlvl,
                                        optarg=solver.options)
    ...
    # _set_port(m.fs.turbine[5].inlet,
    #           m.fs.charge.ess_ip_split.to_turbine)
    ...
    _set_port(m.fs.charge.connector.inlet,
              ...
              # m.fs.charge.ess_ip_split.to_hxc)
...
def build_costing(m, solver=None, optarg={"tol": 1e-8, "max_iter": 300}):
    ...
    print()
    print("****   Starting Costing Initialization ")
    ...
    m.fs.charge.thermal_oil_disjunct.spump_Qgpm = pyo.Expression(
        expr=(m.fs.charge.thermal_oil_disjunct.hxc.side_2.properties_in[0].flow_mass * \
              264.17 * 60 / \
              (m.fs.charge.thermal_oil_disjunct.hxc.side_2.properties_in[0].density)),
        doc="Conversion of solar salt flow mass to volumetric flow in gal per min"
    )
    ...
    m.fs.charge.thermal_oil_disjunct.spump_sf = pyo.Expression(
        expr=m.fs.charge.thermal_oil_disjunct.spump_Qgpm * (m.fs.charge.spump_head ** 0.5),
        doc="Pump size factor"
    )
    m.fs.charge.thermal_oil_disjunct.pump_CP = pyo.Expression(
        expr=(
            ...
            exp(
                9.2951
                - 0.6019 * log(m.fs.charge.thermal_oil_disjunct.spump_sf)
                + 0.0519 * ((log(m.fs.charge.thermal_oil_disjunct.spump_sf))**2)
            )
        ),
        doc="Salt pump base (purchase) cost in $"
    )
    ...
    m.fs.charge.thermal_oil_disjunct.motor_CP = pyo.Expression(
        expr=(
            ...
            exp(
                5.4866
                + 0.13141 * log(m.fs.charge.thermal_oil_disjunct.motor_pc)
                + 0.053255 * ((log(m.fs.charge.thermal_oil_disjunct.motor_pc))**2)
                + 0.028628 * ((log(m.fs.charge.thermal_oil_disjunct.motor_pc))**3)
                - 0.0035549 * ((log(m.fs.charge.thermal_oil_disjunct.motor_pc))**4)
            )
        ),
        doc="Salt Pump's Motor Base Cost in $"
    )
    ...
    def rule_oil_tank_insulation_cost(b):
        return (m.fs.charge.thermal_oil_disjunct.costing.tank_insulation_cost ==
    ...
    def rule_oil_tank_foundation_cost(b):
        return (m.fs.charge.thermal_oil_disjunct.costing.tank_foundation_cost ==
...
def add_bounds(m):
    ...
    for oil_hxc in [m.fs.charge.thermal_oil_disjunct.hxc]:
        ...
        oil_hxc.heat_duty.setub(300e6)
        oil_hxc.shell.heat.setlb(-300e6)
        oil_hxc.shell.heat.setub(0)
        oil_hxc.tube.heat.setlb(0)
        oil_hxc.tube.heat.setub(300e6)
        ...
        oil_hxc.costing.purchase_cost.setub(1e8) # increased value
    ...
    m.fs.charge.capital_cost.setlb(0)
...
def run_nlps(m,
    ...
    if source == "vhp":
        ...
        m.fs.charge.ip_source_disjunct.indicator_var.fix(False)
    elif source == "hp":
        ...
        m.fs.charge.vhp_source_disjunct.indicator_var.fix(False)
        m.fs.charge.hp_source_disjunct.indicator_var.fix(False)
        m.fs.charge.ip_source_disjunct.indicator_var.fix(True)
    else:
...
def print_model(nlp_model, nlp_data):
    ...
    if nlp_model.fs.charge.solar_salt_disjunct.indicator_var.value == 1:
        ...
        print('        No more alternatives in Disjunction 1')
    ...
    if nlp_model.fs.charge.vhp_source_disjunct.indicator_var.value == 1:
        ...
        print('        No more alternatives in Disjunction 2')

    print('')
    print('        Cooler heat duty in MW',
          nlp_model.fs.charge.cooler.heat_duty[0].value * 1e-6)
    for k in nlp_model.set_turbine_splitter:
        print('        Turbine {} split fraction 1: {:.6f}'.
              format(k, value(nlp_model.fs.turbine_splitter[k].split_fraction[0.0, "outlet_1"])))
    for k in nlp_model.set_turbine_splitter:
        print('        Turbine {} split fraction 2: {:.6f}'.
              format(k, value(nlp_model.fs.turbine_splitter[k].split_fraction[0.0, "outlet_2"])))
    print('       ___________________________________________')
    print('')
...
def print_results(m, results):
    ...
    print('')
    if m.fs.charge.solar_salt_disjunct.indicator_var.value:
    ...
    elif m.fs.charge.hitec_salt_disjunct.indicator_var.value:
...
def model_analysis(m, solver):
    ...
    m.fs.charge.ess_ip_split.split_fraction[0, "to_hxc"].unfix()
