...
"""
This is a NLP model for operational scheduling of an ultra
supercritical thermal power plant integrated with a thermal energy storage.

The thermal energy storage design is fixed from a design study, as follows:
    (1) Storage medium: Solar Salt
    (2) Charge steam source: HP turbine inlet
    (3) Discharge condensate source: Boiler feed pump
...
def create_storage_model(m, method=None, max_power=None):
    ...
    # Heat transfer coefficient constraint
...
def _make_constraints(m, method=None, max_power=None):
    ...
    m.fs.max_boiler_duty = Param(
        initialize=1144,
        mutable=False,
        doc='Max duty of the boiler at max charge and max power')
...
def build_costing(m, solver=None, optarg={"tol": 1e-8, "max_iter": 300}):
    ...
    # Annual operating cost ($/y) is the fuel cost (if no cooler)
    def op_cost_rule(b):
...
def main(method=None, max_power=None, load_from_file=None):
    ...
    if load_from_file is not None:
        ...
        m = create_storage_model(m, method=method, max_power=max_power)
...
def model_analysis(m, solver, power=None,
                   max_power=None, tank_scenario=None, fix_power=None):
    ...
    if tank_scenario == "hot_empty":
        ...
        print('Unrecognized scenario! Try hot_empty, hot_full, hot_half_full')
