...
def create_charge_model(m):
    ...
    # Global data
    m.p_max = 436 # in Mw
    m.p_min = int(0.65 * m.p_max) # 283 MW
    m.salt_hot_temperature = 831 # from base charge case at p_max = 436 MW in ...disj_mod.py
    m.storage_heat_duty = 150*1e6 # maximum heat duty in storahe heat exchanger

    # Maximum values calculated from no_storage mode (...disj_mod.py)
    # for p_max = 436MW, and minimum values from no_storage mode
    # (...disj_mod.py) p_min = 283MW
    m.boiler_flow_max = m.main_flow # in mol/s
    m.boiler_flow_min = 11795 # in mol/s
    m.boiler_heat_max = 916e6 # in mol/s
    m.boiler_heat_min = 586e6 # in MW
    ...
    m.fs.salt_amount = Var(
        m.fs.time,
        initialize=6739292*2, # multiplied by 2 so it can have enough for one discharge cycle
    ...
    # Add global variables to save data from selected operation mode
...
def no_storage_mode_disjunct_equations(disj):
    ...
    m.fs.no_storage_mode_disjunct.eq_salt_amount_in_storage = Constraint(
        expr=m.fs.salt_storage[0] == 0
    )
    m.fs.no_storage_mode_disjunct.eq_steam_flow = Constraint(
        expr=m.fs.steam_flow == m.fs.boiler.inlet.flow_mol[0]
    )
...
def charge_mode_disjunct_equations(disj):
    ...
    #     expr=m.fs.charge_mode_disjunct.hxc.heat_duty[0] == m.storage_heat_duty
    # )
    ...
    m.fs.charge_mode_disjunct.eq_steam_flow_to_storage = Constraint(
        expr=m.fs.steam_flow == m.fs.charge_mode_disjunct.ess_hp_split.to_hxc.flow_mol[0]
    )
...
def discharge_mode_disjunct_equations(disj):
    ...
    #     expr=m.fs.discharge_mode_disjunct.hxd.heat_duty[0] == m.storage_heat_duty
    # )
    ...
    m.fs.discharge_mode_disjunct.eq_salt_amount_in_discharge_storage = Constraint(
        expr=m.fs.salt_storage[0] == m.fs.discharge_mode_disjunct.hxd.inlet_1.flow_mass[0] * 3600
    )
    ...
    # Add constraint to save the steam flow as a global variable
    m.fs.discharge_mode_disjunct.eq_steam_flow_to_storage = Constraint(
        expr=m.fs.steam_flow == m.fs.discharge_mode_disjunct.ess_bfp_split.to_hxd.flow_mol[0]
    )
...
def add_bounds(m):
    ...
    m.fs.plant_power_out[0].setlb(m.p_min - 100)
    m.fs.plant_power_out[0].setub(m.p_max)
    ...
    for unit_k in [m.fs.booster]:
        ...
        unit_k.inlet.flow_mol[:].setub(m.boiler_flow_max)  # mol/s
        unit_k.outlet.flow_mol[:].setlb(0)  # mol/s
        unit_k.outlet.flow_mol[:].setub(m.boiler_flow_max)  # mol/s

    # Turbine splitters flow
    for k in m.set_turbine_splitter:
        m.fs.turbine_splitter[k].inlet.flow_mol[:].setlb(0)
        m.fs.turbine_splitter[k].inlet.flow_mol[:].setub(m.boiler_flow_max)
        m.fs.turbine_splitter[k].outlet_1.flow_mol[:].setlb(0)
        m.fs.turbine_splitter[k].outlet_1.flow_mol[:].setub(m.boiler_flow_max)
        m.fs.turbine_splitter[k].outlet_2.flow_mol[:].setlb(0)
        m.fs.turbine_splitter[k].outlet_2.flow_mol[:].setub(m.boiler_flow_max)
    ...
    for unit_in_charge in [m.fs.charge_mode_disjunct]:
        ...
        unit_in_charge.hxc.inlet_1.flow_mol.setub(0.2 * m.boiler_flow_max)
        ...
        unit_in_charge.hxc.outlet_1.flow_mol.setub(0.2 * m.boiler_flow_max)
        ...
        unit_in_charge.hxc.delta_temperature_in.setub(81)
        ...
        for unit_k in [unit_in_charge.hx_pump,
            ...
            unit_k.inlet.flow_mol.setub(0.2*m.boiler_flow_max)
            unit_k.outlet.flow_mol.setlb(0)
            unit_k.outlet.flow_mol.setub(0.2*m.boiler_flow_max)
        ...
        unit_in_charge.ess_hp_split.to_hxc.flow_mol[:].setub(0.2 * m.boiler_flow_max)
        unit_in_charge.ess_hp_split.to_turbine.flow_mol[:].setlb(0)
        unit_in_charge.ess_hp_split.to_turbine.flow_mol[:].setub(m.boiler_flow_max)
        ...
        unit_in_charge.ess_hp_split.inlet.flow_mol[:].setub(m.boiler_flow_max)

        # Recycle mixer
        unit_in_charge.recycle_mixer.from_bfw_out.flow_mol.setlb(0)
        unit_in_charge.recycle_mixer.from_bfw_out.flow_mol.setub(m.boiler_flow_max)
        unit_in_charge.recycle_mixer.from_hx_pump.flow_mol.setlb(0)
        unit_in_charge.recycle_mixer.from_hx_pump.flow_mol.setub(0.2 * m.boiler_flow_max)
        unit_in_charge.recycle_mixer.outlet.flow_mol.setlb(0)
        unit_in_charge.recycle_mixer.outlet.flow_mol.setub(m.boiler_flow_max)
    ...
    for unit_in_discharge in [m.fs.discharge_mode_disjunct]:
        ...
        unit_in_discharge.hxd.inlet_2.flow_mol.setub(0.2 * m.boiler_flow_max)
        ...
        unit_in_discharge.hxd.outlet_2.flow_mol.setub(0.2 * m.boiler_flow_max)
        ...
        unit_in_discharge.ess_bfp_split.to_hxd.flow_mol[:].setub(0.2 * m.boiler_flow_max)
        unit_in_discharge.ess_bfp_split.to_fwh8.flow_mol[:].setlb(0)
        unit_in_discharge.ess_bfp_split.to_fwh8.flow_mol[:].setub(m.boiler_flow_max)
        ...
        unit_in_discharge.ess_bfp_split.inlet.flow_mol[:].setub(m.boiler_flow_max)

        # ES Turbine
        unit_in_discharge.es_turbine.inlet.flow_mol[:].setlb(0)
        unit_in_discharge.es_turbine.inlet.flow_mol[:].setub(0.2 * m.boiler_flow_max)
        unit_in_discharge.es_turbine.outlet.flow_mol[:].setlb(0)
        unit_in_discharge.es_turbine.outlet.flow_mol[:].setub(0.2 * m.boiler_flow_max)
...
def print_model(nlp_model, nlp_data):
    ...
    if nlp_model.fs.charge_mode_disjunct.indicator_var.value == 1:
        ...
        nlp_model.fs.charge_mode_disjunct.hxc.display()
        ...
        nlp_model.fs.discharge_mode_disjunct.hxd.display()
...
def model_analysis(m, solver, cycle=None, power=None):
    ...
    m.fs.charge_mode_disjunct.hxc.area.fix(1904) # in m2
    m.fs.charge_mode_disjunct.hxc.outlet_2.temperature[0].fix(m.salt_hot_temperature)
    m.fs.discharge_mode_disjunct.hxd.area.fix(1095) # in m2
    m.fs.discharge_mode_disjunct.hxd.inlet_1.temperature[0].fix(m.salt_hot_temperature)
    m.fs.discharge_mode_disjunct.hxd.outlet_1.temperature[0].fix(513.15)
    ...
    m.fs.previous_salt_inventory_hot[0].fix(6500000)
    ...
    def constraint_salt_inventory_hot(b, t):
        return (
            ...
            + b.charge_mode_disjunct.binary_indicator_var * m.fs.salt_storage[0]
            - b.discharge_mode_disjunct.binary_indicator_var * m.fs.salt_storage[0])
    ...
    def constraint_salt_previous_inventory(b, t):
        return (
            b.previous_salt_inventory_hot[t] + b.previous_salt_inventory_cold[t] == b.salt_amount[t]
        )
    ...
    def production_cons_with_storage(b, t):
        return (
            (-1 * sum(b.turbine[p].work_mechanical[t]
             ...
             - b.hx_pump_work[0]
             + ((-1) * b.discharge_turbine_work[0]) # add es turbine work
            ) ==
            b.plant_power_out[t] * 1e6 * (pyunits.W/pyunits.MW)
        )

    @m.fs.Constraint(m.fs.time)
    def constraint_power_production_and_demand(b, t):
        return b.plant_power_out[t] >= power

    # Add new calculated flow bounds to boiler
    m.fs.boiler_flow_lb = Constraint(
        expr=m.fs.boiler.inlet.flow_mol[0] >= m.boiler_flow_min)
    m.fs.boiler_flow_ub = Constraint(
        expr=m.fs.boiler.inlet.flow_mol[0] <= m.boiler_flow_max)
