...
def add_bounds(m, source=None):
    ...
    for hxd in [m.fs.discharge.hxd]:
        ...
        hxd.delta_temperature_in.setub(80)
        hxd.delta_temperature_out.setub(80)
    ...
    m.fs.discharge.capital_cost.setub(None)
    m.fs.discharge.capital_cost_upper_bound = Constraint(
        expr=m.fs.discharge.capital_cost <= 1e7
    )
...
if __name__ == "__main__":
    ...
    m = model_analysis(m, solver, heat_duty=148.5, source="fwh4")
    # results = solver.solve(
    #     m,
    #     tee=True,
    #     symbolic_solver_labels=True,
