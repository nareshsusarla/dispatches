...
def solar_salt_disjunct_equations(disj):
    ...
    solar salt as the storage medium in charge heat exchanger
    """
    ...
    # Heat transfer Coefficient calculations for solar salt heat exchanger

    # # Scaling factors
    # for salt_hxc in [m.fs.charge.solar_salt_disjunct.hxc]:
    #     iscale.set_scaling_factor(salt_hxc.area, 1e-2)
    #     iscale.set_scaling_factor(
    #         salt_hxc.overall_heat_transfer_coefficient, 1e-3)
    ...
    # Connecting the cooler to the solar charge heat exchanger
...
def hitec_salt_disjunct_equations(disj):
    ...
    hitec salt as the storage medium in charge heat exchanger
    """
    ...
    # Scaling factors
    # for salt_hxc in [m.fs.charge.hitec_salt_disjunct.hxc]:
    #     iscale.set_scaling_factor(salt_hxc.area, 1e-2)
    #     iscale.set_scaling_factor(
    #         salt_hxc.overall_heat_transfer_coefficient, 1e-3)
    #     iscale.set_scaling_factor(salt_hxc.shell.heat, 1e-6)
    ...
    # Connecting the cooler to the hitec charge heat exchanger
...
def thermal_oil_disjunct_equations(disj):
    """
    Block of equations for disjunct 2 for the selection of 
    thermal oil as the storage medium in charge heat exchanger
    """
    ...
    # Connecting the cooler to thermal oil charge heat exchanger
...
def set_model_input(m):
    ...
    # Oil conditions: from Andres's model
    m.fs.charge.thermal_oil_disjunct.hxc.overall_heat_transfer_coefficient.fix(432.677)
    m.fs.charge.thermal_oil_disjunct.hxc.inlet_2.flow_mass[0].fix(600)
    m.fs.charge.thermal_oil_disjunct.hxc.inlet_2.temperature[0].fix(200 + 273.15)
...
def set_scaling_factors(m):
    ...
    # Data from Andres's model
    for oil_hxc in [m.fs.charge.thermal_oil_disjunct.hxc]:
        iscale.set_scaling_factor(oil_hxc.area, 1e-3)
        iscale.set_scaling_factor(
            oil_hxc.overall_heat_transfer_coefficient, 1e-2)
        # iscale.set_scaling_factor(oil_hxc.shell.heat, 1e-6)
        iscale.set_scaling_factor(oil_hxc.tube.heat, 1e-6)
        iscale.set_scaling_factor(oil_hxc.tube.properties_in[0].flow_mass, 1e-2)

    iscale.set_scaling_factor(
        m.fs.charge.hx_pump.control_volume.work, 1e-6)
    iscale.set_scaling_factor(
        m.fs.charge.cooler.control_volume.heat, 1e-6)
    iscale.set_scaling_factor(
        m.fs.charge.connector.control_volume.heat, 1e-1)

    # return m

def initialize(m, fileinput=None,
    ...
    #-------- modified by esrawli
    #  Solar salt charge heat exchanger initialization
    ...
    _set_port(m.fs.charge.thermal_oil_disjunct.hxc.inlet_1,
              m.fs.charge.connector.outlet)
    # m.fs.charge.thermal_oil_disjunct.hxc.initialize(outlvl=outlvl) # from Andres's model
    ...
    m.fs.charge.thermal_oil_disjunct.hxc.initialize(outlvl=outlvl) # from our model
    #--------
...
def add_bounds(m):
    ...
    """
    print('Main flow (mol/s)= ', m.main_flow)
    ...
    for oil_hxc in [m.fs.charge.thermal_oil_disjunct.hxc]:
        ...
        oil_hxc.heat_duty.setub(200e6)  
        oil_hxc.shell.heat.setlb(-200e6)
        oil_hxc.shell.heat.setub(0)  
        oil_hxc.tube.heat.setlb(0)  
        oil_hxc.tube.heat.setub(200e6) 
        # oil_hxc.tube.properties_in[0].enthalpy_mass.setlb(0)
        # oil_hxc.tube.properties_in[0].\
        #     enthalpy_mass.setub(1.5e6)
        # oil_hxc.tube.properties_out[0].enthalpy_mass.setlb(0)
        # oil_hxc.tube.properties_out[0].\
        #     enthalpy_mass.setub(1.5e6)
...
def model_analysis(m, solver):
    ...
    # Fixing the plant power
    m.fs.plant_power_out.fix(400)

    # Unfixing the boiler flow since the power is fixed
    ...
    # Unfix solar and hitec in charge heat exchanger salt flow,
    # steam flow, temperature, and area 
    for salt_hxc in [m.fs.charge.solar_salt_disjunct.hxc,
                     m.fs.charge.hitec_salt_disjunct.hxc,
    ...
    if m.fs.charge.solar_salt_disjunct.indicator_var == 1:
    ...
    elif m.fs.charge.hitec_salt_disjunct.indicator_var == 1:
        ...
        print("HXC heat duty: ",
              value(m.fs.charge.thermal_oil_disjunct.hxc.heat_duty[0]) / 1e6)
    print("")
