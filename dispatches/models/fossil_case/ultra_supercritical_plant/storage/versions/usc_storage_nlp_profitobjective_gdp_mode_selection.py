...
def create_charge_model(m):
    ...
    """
    m.Pmax = 436  # Plant design power in MW
    m.Pmin = 0.6 * m.Pmax  # Min power allowed
    ...
    m.fs.charge_mode_disjunct = Disjunct(
        rule=charge_mode_disjunct_equations)
    m.fs.discharge_mode_disjunct = Disjunct(
        rule=discharge_mode_disjunct_equations)
    m.fs.nostorage_mode_disjunct = Disjunct(
        rule=nostorage_mode_disjunct_equations)
...
def nostorage_mode_disjunct_equations(disj):
    ...
    # # Fix discharge heat exchanger heat duty to a very small value
    # m.fs.nostorage_mode_disjunct.hxd_heat_duty = Constraint(
    #     expr=m.fs.hxd.heat_duty[0] == 0.1*1e6
    # )
    ...
    # Fix data for discharge heat exchanger since all data for both
    # heat exchangers was unfixed in model_analysis
    m.fs.nostorage_mode_disjunct.hxd_inlet_dummy_flow = Constraint(
        expr=m.fs.hxd.inlet_1.flow_mass[0] == 1
    )
    m.fs.nostorage_mode_disjunct.hxc_inlet_dummy_flow = Constraint(
...
def build_costing(m, solver=None, optarg={"tol": 1e-8, "max_iter": 300}):
    ...
    #     expr=(m.fs.if_charge * m.fs.salt_amount_hxc +
    #           (1 - m.fs.if_charge) * m.fs.salt_amount_hxd),
    #     doc="Total Solar salt inventory flow in kg per s"
    # )
    ...
    m.fs.spump_sf = Expression(
        expr=(m.fs.spump_Qgpm
              * (m.fs.spump_head ** 0.5)),
        doc="Pump size factor"
    )
    m.fs.pump_CP = Expression(
        expr=(
    ...
    # Motor purchase cost
    log_motor_pc = log(m.fs.motor_pc)
    m.fs.motor_CP = Expression(
        expr=(
            m.fs.spump_motorFT *
            exp(
                5.4866
    ...
    def solar_spump_purchase_cost_rule(b):
        return (
            ...
            m.fs.num_of_years == (m.fs.pump_CP + m.fs.motor_CP) *
            2 * (m.CE_index / 394)  # multiplied by 2 to include discharge
        )
    m.fs.spump_purchase_cost_eq = Constraint(
...
def add_bounds(m):
    ...
    # m.fs.hxc.tube.properties_in[:].enthalpy_mass.setlb(
    #     m.fs.charge.salt_enthalpy_mass_min / m.factor)
    # m.fs.hxc.tube.properties_in[:].enthalpy_mass.setub(
    #     m.fs.charge.salt_enthalpy_mass_max * m.factor)
    # m.fs.hxc.tube.properties_out[:].enthalpy_mass.setlb(
    #     m.fs.charge.salt_enthalpy_mass_min / m.factor)
    # m.fs.hxc.tube.properties_out[:].enthalpy_mass.setub(
    #     m.fs.charge.salt_enthalpy_mass_max * m.factor)
    ...
    # Add calculated bounds
    # m.fs.hxd.tube.properties_in[:].enthalpy_mass.setlb(
    #     m.fs.charge.salt_enthalpy_mass_min / m.factor)
    # m.fs.hxd.tube.properties_in[:].enthalpy_mass.setub(
    #     m.fs.charge.salt_enthalpy_mass_max * m.factor)
    # m.fs.hxd.tube.properties_out[:].enthalpy_mass.setlb(
    #     m.fs.charge.salt_enthalpy_mass_min / m.factor)
    # m.fs.hxd.tube.properties_out[:].enthalpy_mass.setub(
    #     m.fs.charge.salt_enthalpy_mass_max * m.factor)
    ...
    for unit_k in [m.fs.hx_pump,
        ...
        unit_k.outlet.enth_mol.setlb(0)
        unit_k.outlet.enth_mol.setub(50000)
    m.fs.cooler.heat_duty.setlb(-1e7)
...
def model_analysis(m, solver, cycle=None):
    ...
    m.fs.plant_power_out.fix(m.Pmin)
    ...
    if cycle == "charge":
        ...
        m.fs.nostorage_mode_disjunct.indicator_var.fix(False)
    elif cycle == "discharge":
        m.fs.charge_mode_disjunct.indicator_var.fix(False)
        m.fs.discharge_mode_disjunct.indicator_var.fix(True)
        m.fs.nostorage_mode_disjunct.indicator_var.fix(False)
    elif cycle == "nostorage":
        m.fs.charge_mode_disjunct.indicator_var.fix(False)
        m.fs.discharge_mode_disjunct.indicator_var.fix(False)
        m.fs.nostorage_mode_disjunct.indicator_var.fix(True)
    else:
